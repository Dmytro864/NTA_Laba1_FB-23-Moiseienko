...
def factor(n):
    ...
    def recursive(n):
        ...
        d = brilhart(n)
        if d:
            report(d, "Метод Брілхарта")
            recursive(n // d)
        else:
            print("Я не можу знайти канонічний розклад числа :(")

    recursive(n)
    print(f"\nКанонічний розклад: {sorted(factors)}")
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        try:
            ...
            print("Ввести ціле число як аргумент.")
    else:
        try:
            num = int(input("Ввести число для факторизації: "))
            factor(num)
        except ValueError:
            print("Ввести коректне число")
